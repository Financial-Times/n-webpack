...
`, '', { match: /\.scss$/ })
		...
		} else if (this.opts.cssVariant === 'without-n-ui') {
			this.config.plugins.push(
				new EntryWrap('$n-ui-include-core: false', '', { match: /\.scss$/ })
			);
		}
...
module.exports = function buildConfig (options, buildVariants) {
	if (buildVariants) {
		return variants(options).map(options => new Configurator(options).exec());
	} else {
