...
function constructVariants (options) {
	return [
		...
		Object.assign(clone(options), {
			language: 'scss',
			cssVariant: 'ie8',
			entry: filterEntryKeys(options.entry, /main.*\.css$/, name => name.replace(/\.css$/, '-ie8.css')),
			withHeadCss: false
		}),
		Object.assign(clone(options), {
			language: 'js',
			externals: {'n-ui': true},
			entry: filterEntryKeys(options.entry, /main\.js$/, name => name.replace(/\.js$/,'-without-n-ui.js'))
		})
	]
}
