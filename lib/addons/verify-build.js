...
function VerifyBuild (env) {
	return function () {
		this.plugin('done', stats => {

			// In prod we care about __about.json etc, in dev we may not generate them every time so scope to css & js only
			const rx = env === 'prod' ? /^\.?\/?public/ : /^\.?\/?public.*(css|js)$/;
